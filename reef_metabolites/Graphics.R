...
data_nmds_spec$Species<- factor(data_nmds_spec$Species, levels = c("Montipora digitata", "Pocillopora verrucosa",
                                                                   "Xenia umbellata", "Sclerophytum wanannense",
                                                                   "Caulerpa brachypus","Peyssonnelia sp.",
...
# ggsave("graphs/Figure2.png", width=11, height = 5, limitsize = FALSE, dpi = 700, arranged_overview_nodes)
...
mono_species <- ggplot(data_nmds_mono, aes(x=sites.NMDS1, y=sites.NMDS2, col = Organism_group, shape = Species)) +
  ...
  scale_shape_manual(name = "Species", label=c("Montipora digitata" = expression(italic("Montipora digitata")),
                                               ...
                                               "Xenia umbellata" = expression(italic("Xenia umbellata")),
                                               "Sclerophytum wanannense" = expression(italic("Sclerophytum wanannense")),
                                               "Caulerpa sp." = expression(italic("Caulerpa brachypus")),
  ...
  ylab("NMDS2") +
  #stat_ellipse() +
  annotate(geom = "text", x = -0.8, y = 0.4, size = 3,
           label = "Monoculture
  2D Stress = 0.15",
           hjust = 0) +
...
poly_species <- ggplot(data_nmds_poly, aes(x=sites.NMDS1, y=sites.NMDS2, col = Organism_group, shape = Species)) +
  ...
  scale_shape_manual(name = "Species", label=c("Montipora digitata" = expression(italic("Montipora digitata")),
                                               "Pocillopora verrucosa" = expression(italic("Pocillopora verrucosa")),
                                               "Xenia sp." = expression(italic("Xenia umbellata")),
                                               "Sinularia sp." = expression(italic("Sclerophytum wanannense")),
                                               "Caulerpa sp." = expression(italic("Caulerpa brachypus")),
                                               "Peyssonnelia sp." = expression(paste(italic("Peyssonnelia")," sp.")),
                                               "Haliclona cnidata" = expression(italic("Haliclona cnidata")),
                                               "control" = expression("control")),
...
semi_species <- ggplot(data_nmds_semi, aes(x=sites.NMDS1, y=sites.NMDS2, col = Organism_group, shape = Species)) +
 ...
 # stat_ellipse() +
  theme_classic() +
  theme(panel.border = element_rect(colour = "black", fill=NA)) +
  annotate(geom = "text", x = 0.75, y = 0.35, size = 3,
...
arranged_treatment
#ggsave("graphs/Figure3.png", width=12, height = 5, limitsize = FALSE, dpi = 700, arranged_treatment)
...
cau <- all %>% 
  ...
  subset(Species != "Sclerophytum wanannense") %>%
  subset(Species != "Xenia umbellata") %>%
...
#Peyssonnelia sp.
pey <- all %>% 
  subset(Species != "Caulerpa brachypus") %>%
  subset(Species != "Sclerophytum wanannense") %>%
  subset(Species != "Xenia umbellata") %>%
  subset(Species != "Montipora digitata") %>%
  subset(Species != "Pocillopora verrucosa") %>%
...
pey_treat <- ggplot(data_nmds_pey, aes(x=sites.NMDS1, y=sites.NMDS2, color = Treatment, shape = Treatment)) +
  ...
  ylab("NMDS2") +
  theme(panel.border = element_rect(colour = "black", fill=NA)) +
  annotate(geom = "text", x = -0.8, y = 0.23, size = 3,
...
sin_treat <- ggplot(data_nmds_sin, aes(x=sites.NMDS1, y=sites.NMDS2, color = Treatment, shape = Treatment)) +
  geom_point(aes(colour=Treatment, fill=Treatment), size = 3, color = "black") +
  scale_fill_manual(values=c("#99FFFF", "#3333FF", "#000033", "#999999")) +
 # scale_colour_manual(values=c("#99FFFF", "#3333FF", "#000033", "#999999")) +
  scale_shape_manual(values=c(21, 21, 21, 4)) +
  theme_classic() +
  xlab("NMDS1") +
  ylab("NMDS2") +
 # stat_ellipse() +
  theme(panel.border = element_rect(colour = "black", fill=NA)) +
  annotate(geom = "text", x = -0.8, y = 0.21, size = 3,
           label = "S. wanannense
  2D Stress = 0.10",
           hjust = 0)
...
mdi_treat <- ggplot(data_nmds_mdi, aes(x=sites.NMDS1, y=sites.NMDS2, color = Treatment, shape = Treatment)) +
  ...
  ylab("NMDS2") +
  theme(panel.border = element_rect(colour = "black", fill=NA)) +
  annotate(geom = "text", x = -0.7, y = 0.24, size = 3,
           label = "M. digitata
  2D Stress = 0.12",
           hjust = 0)
...
pve_treat <- ggplot(data_nmds_pve, aes(x=sites.NMDS1, y=sites.NMDS2, color = Treatment, shape = Treatment)) +
  ...
  ylab("NMDS2") +
  theme(panel.border = element_rect(colour = "black", fill=NA)) +
  annotate(geom = "text", x = -0.65, y = 0.19, size = 3,
           label = "P. verrucosa
  2D Stress = 0.14",
           hjust = 0)
...
hcn_treat <- ggplot(data_nmds_hcn, aes(x=sites.NMDS1, y=sites.NMDS2, color = Treatment, shape = Treatment)) +
  ...
  ylab("NMDS2") +
  theme(panel.border = element_rect(colour = "black", fill=NA)) +
  annotate(geom = "text", x = -0.48, y = 0.25, size = 3,
           label = "H. cnidata
  2D Stress = 0.15",
           hjust = 0)
...
ggsave("graphs/Figure4.png", width=11, 
       height = 15, limitsize = FALSE, dpi = 700, arranged_single_species_nodes)
...
Species <- ggplot(data_nmds_spec_nc, aes(x=sites.NMDS1, y=sites.NMDS2, color = Species)) +
  ...
  theme(legend.text = element_text(size = 8, face = "italic"),
        legend.position = "bottom") +
  annotate(geom = "text", x = -0.8, y = 0.37, size = 2.5, 
           label = "Species 
  2D Stress = 0.22
  Cau ~ Hcn p = 0.006**
  Cau ~ Pey p = 0.014 *
...
treat <- ggplot(data_nmds_treat_nc, aes(x=sites.NMDS1, y=sites.NMDS2, color = Treatment)) +
  ...
  theme(legend.text = element_text(size = 8),
        legend.position = "bottom") +
  annotate(geom = "text", x = -0.6, y = 0.44, size = 2.5,
           label = "Treatment
  2D Stress = 0.22
  p > 0.05",
           hjust = 0) +
...
arranged_overview
ggsave("graphs/S1.png", width=11, height = 5, limitsize = FALSE, dpi = 700, arranged_overview)
...
mono_species <- ggplot(data_nmds_mono, aes(x=sites.NMDS1, y=sites.NMDS2, col = Organism_group, shape = Species)) +
  ...
  scale_shape_manual(name = "Species", label=c("Montipora digitata" = expression(italic("Montipora digitata")),
                                               ...
                                               "Xenia umbellata" = expression(italic("Xenia umbellata")),
                                               "Sclerophytum wanannense" = expression(italic("Sclerophytum wanannense")),
                                               "Caulerpa brachypus" = expression(italic("Caulerpa brachypus")),
                                               "Peyssonnelia sp." = expression(paste(italic("Peyssonnelia")," sp.")),
                                               "Haliclona cnidata" = expression(italic("Haliclona cnidata"))),
...
arranged_treatment
ggsave("graphs/S2.png", width=11, height = 5, limitsize = FALSE, dpi = 700, arranged_treatment)
...
#___________________________________________________________________
#Caulerpa sp. 
cau <- all %>% subset(Species=="Caulerpa brachypus")
cau_data <- cau %>% as.data.frame()
rownames(cau_data) <- cau_data$Fragment_ID
...
#______________________________________
# Xenia
xen <- all %>% subset(Species=="Xenia umbellata")
xen_data <- xen %>% as.data.frame()
rownames(xen_data) <- xen_data$Fragment_ID
...
sin <- all %>% subset(Species=="Sclerophytum wanannense") 
sin_data <- sin %>% as.data.frame()
rownames(sin_data) <- sin_data$Fragment_ID
...
ggsave("graphs/S3.png", width=11, 
       height = 15, limitsize = FALSE, dpi = 700, arranged_single_species)
